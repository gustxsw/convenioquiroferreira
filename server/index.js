...
const initializeDatabase = async () => {
  try {
    ...
    // Migrate scheduling_access: add columns added after the table was first created
    await pool.query(`
      DO $$
      BEGIN
        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'scheduling_access' AND column_name = 'schedule_balance'
        ) THEN
          ALTER TABLE scheduling_access ADD COLUMN schedule_balance INTEGER DEFAULT 0;
        END IF;

        IF NOT EXISTS (
          SELECT 1 FROM information_schema.columns
          WHERE table_name = 'scheduling_access' AND column_name = 'updated_at'
        ) THEN
          ALTER TABLE scheduling_access ADD COLUMN updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP;
        END IF;
      END $$;
    `);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // Admin é sempre visão global. financeiro_agenda SEMPRE é escopado: só vê
      // os profissionais vinculados a ele E somente se estiver marcado como
...
// Lista usuários com perfil financeiro_agenda (para o admin escolher quem vira
// parceiro, sem precisar digitar o ID na mão). Inclui a flag is_agenda_partner
// para o front filtrar quem já é parceiro.
app.get(
  "/api/admin/agenda-partners/eligible-users",
  authenticate,
  authorize(["admin"]),
  async (req, res) => {
    try {
      const result = await pool.query(`
        SELECT id, name, email, is_agenda_partner
        FROM users
        WHERE 'financeiro_agenda' = ANY(roles)
        ORDER BY name
      `);
      res.json(
        result.rows.map((row) => ({
          ...
          name: row.name,
          email: row.email,
          is_agenda_partner: Boolean(row.is_agenda_partner),
        }))
      );
    } catch (error) {
      console.error("❌ Error listing eligible agenda partner users:", error);
      res
        .status(500)
        .json({ message: "Erro ao listar usuários financeiro_agenda" });
    }
  }
);
...
app.get(
  ...
  async (req, res) => {
    try {
      ...
      // First, try the current medical_documents table.
...
app.post(
  ...
  async (req, res) => {
    ...
    `,
      ...
      // Build structured content for medical record PDF generation
      ...
      const documentData = await generateDocumentPDF("medical_record", {
        ...
        professionalName: req.user.name || "Profissional de Saúde",
