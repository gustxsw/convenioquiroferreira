...
// ========================================
// CONSULTATION CANCELLATION ROUTES
// ========================================
...
app.put('/api/consultations/:id/cancel', authenticate, async (req, res) => {
  try {
    ...
    console.log('🔄 [CANCEL] Cancelling consultation:', consultationId);
    ...
    if (req.user.currentRole === 'professional' && consultation.professional_id !== req.user.id) {
    ...
    console.log('✅ [CANCEL] Consultation cancelled successfully');
    ...
    console.error('❌ [CANCEL] Error cancelling consultation:', error);
    res.status(500).json({ 
...
app.get('/api/reports/cancelled-consultations', authenticate, async (req, res) => {
  try {
    ...
    console.log('🔄 [CANCEL-REPORT] Fetching cancelled consultations report');
    console.log('🔄 [CANCEL-REPORT] Date range:', { start_date, end_date });
    ...
    let query = `
      ...
      LEFT JOIN users u_cancelled ON c.cancelled_by = u_cancelled.id
    ...
    console.log('🔄 [CANCEL-REPORT] Executing query with params:', queryParams);

    const result = await pool.query(query, queryParams);

    console.log('✅ [CANCEL-REPORT] Cancelled consultations found:', result.rows.length);

    res.json(result.rows);
  } catch (error) {
    console.error('❌ [CANCEL-REPORT] Error fetching cancelled consultations:', error);
    res.status(500).json({ 
      message: error.message || 'Erro ao carregar relatório de consultas canceladas' 
    });
  }
});

// ========================================
// END CONSULTATION CANCELLATION ROUTES
// ========================================
