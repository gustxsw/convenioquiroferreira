  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        u.neighborhood, u.city, u.state, u.zip_code, u.roles,
        u.subscription_status, u.subscription_expiry, u.percentage,
        ...
        u.medical_history, u.current_medications, u.known_allergies, u.insurance_info, u.notes,
        sc.name as category_name,
...
app.post('/api/admin/users', authenticate, authorize(['admin']), async (req, res) => {
  try {
    const {
      name, cpf, email, phone, birth_date, address, address_number, address_complement,
      neighborhood, city, state, zip_code, roles, percentage, category_id, crm, password,
      subscription_status, subscription_expiry, emergency_contact_name, emergency_contact_phone,
      emergency_contact_relationship, medical_history, current_medications, known_allergies,
    ...
    const result = await pool.query(`
      INSERT INTO users (
        name, cpf, email, phone, birth_date, address, address_number, address_complement,
        neighborhood, city, state, zip_code, password, roles, percentage, category_id, crm,
        subscription_status, subscription_expiry, emergency_contact_name, emergency_contact_phone,
        emergency_contact_relationship, medical_history, current_medications, known_allergies,
        insurance_info, notes
...
app.put('/api/admin/users/:id', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    const {
      ...
      neighborhood, city, state, zip_code, roles, subscription_status, subscription_expiry,
      percentage, category_id, crm, emergency_contact_name, emergency_contact_phone,
      emergency_contact_relationship, medical_history, current_medications, known_allergies,
      insurance_info, notes
    } = req.body;
    ...
    const result = await pool.query(`
      UPDATE users 
      SET 
        name = $1, email = $2, phone = $3, birth_date = $4, address = $5, address_number = $6,
        address_complement = $7, neighborhood = $8, city = $9, state = $10, zip_code = $11,
        roles = $12, subscription_status = $13, subscription_expiry = $14, percentage = $15,
        category_id = $16, crm = $17, emergency_contact_name = $18, emergency_contact_phone = $19,
        emergency_contact_relationship = $20, medical_history = $21, current_medications = $22,
        known_allergies = $23, insurance_info = $24, notes = $25, updated_at = CURRENT_TIMESTAMP
      WHERE id = $26
      RETURNING *
    `, [
      name, email || null, phone || null, birth_date || null, address || null, address_number || null,
      address_complement || null, neighborhood || null, city || null, state || null, zip_code || null,
      roles, subscription_status, subscription_expiry || null, percentage, category_id || null,
      crm || null, emergency_contact_name || null, emergency_contact_phone || null,
      emergency_contact_relationship || null, medical_history || null, current_medications || null,
      known_allergies || null, insurance_info || null, notes || null, id
    ]);
  ...
  try {
    const result = await pool.query(`
      SELECT 
        ...
        subscription_expiry, photo_url, category_name, crm, professional_percentage,
...
app.put('/api/users/:id', authenticate, async (req, res) => {
  try {
    ...
    const { 
      name, email, phone, birth_date, address, address_number, 
      address_complement, neighborhood, city, state, currentPassword, 
      newPassword, category_name, crm, professional_percentage, roles
    } = req.body;
    ...
    if (professional_percentage !== undefined) {
      updateFields.push(`professional_percentage = $${paramCount}`);
      values.push(professional_percentage);
      paramCount++;
    }
...
app.post('/api/services', authenticate, authorize(['admin']), async (req, res) => {
  try {
    ...
    console.error('Error creating service:', error);
    if (error.code === '23505') {
      res.status(400).json({ message: 'Já existe um serviço com este nome' });
    } else {
      res.status(500).json({ message: 'Erro interno do servidor' });
    }
  }
  ...
  try {
    ...
    const result = await pool.query(`
      SELECT 
        ...
        billing_amount, payment_reference, activated_at, created_at, updated_at
      FROM dependents 
      WHERE client_id = $1 
      ORDER BY created_at DESC
    `, [client_id]);
  ...
  try {
    ...
    const result = await pool.query(`
      INSERT INTO dependents (
        client_id, name, cpf, birth_date, subscription_status, 
        billing_amount, created_at, updated_at
      )
...
app.put('/api/dependents/:id', authenticate, async (req, res) => {
  try {
    ...
    if (req.user.currentRole !== 'admin' && req.user.id !== dependentCheck.rows[0].client_id) {
      return res.status(403).json({ message: 'Acesso negado' });
    }
...
app.get('/api/consultations/client/:client_id', authenticate, async (req, res) => {
  try {
    const { client_id } = req.params;
...
app.post('/api/private-patients', authenticate, authorize(['professional']), async (req, res) => {
  try {
    ...
    // Check if CPF already exists for this professional (if CPF provided)
    if (cpf) {
      const existingPatient = await pool.query(
...
app.put('/api/private-patients/:id', authenticate, authorize(['professional']), async (req, res) => {
  try {
    ...
    `, [
      name, email, phone, birth_date, address, address_number,
      address_complement, neighborhood, city, state, zip_code, id
    ]);
...
// ===== CRUD ATTENDANCE LOCATIONS =====
// Get attendance locations for professional
app.get('/api/attendance-locations', authenticate, authorize(['professional']), async (req, res) => {
  try {
    const result = await pool.query(`
      SELECT 
        id, name, address, address_number, address_complement, neighborhood,
...
app.put('/api/attendance-locations/:id', authenticate, authorize(['professional']), async (req, res) => {
  try {
    ...
    `, [
      name, address, address_number, address_complement, neighborhood,
      city, state, zip_code, phone, is_default, id
    ]);
