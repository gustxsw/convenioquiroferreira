...
const ManageAgendaPartnersPage: React.FC = () => {
  ...
  const fetchEligibleUsers = async () => {
    try {
      ...
      const response = await fetchWithAuth(
        `${apiUrl}/api/admin/agenda-partners/eligible-users`
      );
  ...
  const openConfigModal = (partner?: Partner) => {
    ...
    // No cadastro de um novo parceiro, carrega a lista de usuários elegíveis.
    if (!partner) fetchEligibleUsers();
  };
            ...
            <form onSubmit={saveConfig} className="p-6 space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Usuário (perfil financeiro_agenda) *
                </label>
                {editingName !== null ? (
                  <input
                    type="text"
                  ...
                  <>
                    ...
                    <p className="text-xs text-gray-500 mt-1">
                      Apenas usuários com o perfil "financeiro_agenda" aparecem
                      aqui. Atribua o perfil na tela "Usuários" se faltar alguém.
                    </p>
                  </>
                )}
              </div>
