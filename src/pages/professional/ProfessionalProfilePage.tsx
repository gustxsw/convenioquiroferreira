...
const ProfessionalProfilePage: React.FC = () => {
  ...
  const [profileTab, setProfileTab] = useState<"dados" | "convenio" | "planos">("dados");
  ...
  const addInsurance = async () => {
    ...
    try {
      ...
      if (!r.ok) { const d = await r.json(); setInsuranceError(d.message || "Erro ao adicionar"); return; }
      const created = await r.json();
      setInsurances((prev) => [...prev, created].sort((a, b) => a.name.localeCompare(b.name)));
      setNewInsurance("");
    } catch { setInsuranceError("Erro ao adicionar convênio"); }
    finally { setInsuranceLoading(false); }
  };

  const removeInsurance = async (id: number) => {
    try {
      await fetchWithAuth(`${getApiUrl()}/api/professional/insurances/${id}`, { method: "DELETE" });
      setInsurances((prev) => prev.filter((i) => i.id !== id));
    } catch { setInsuranceError("Erro ao remover convênio"); }
  };
  ...
  return (
    <div>
      ...
      <div className="flex flex-wrap gap-2 mb-6 border-b border-gray-200 pb-1">
        <button
          type="button"
          onClick={() => { setProfileTab("dados"); window.history.replaceState(null, "", location.pathname); }}
        ...
        >
          ...
          {insurances.length > 0 && (
            <span className="rounded-full bg-red-100 text-red-700 text-[10px] font-semibold px-1.5 py-px">
              {insurances.length}
            </span>
          )}
        </button>
        {isAgendaOnlyProfile && (
          <button
            type="button"
            onClick={() => { setProfileTab("convenio"); window.history.replaceState(null, "", `${location.pathname}#convenio`); }}
            className={`inline-flex items-center gap-2 px-4 py-2 text-sm font-medium rounded-t-lg border-b-2 -mb-px transition-colors ${
              profileTab === "convenio"
                ? "border-red-600 text-red-600 bg-red-50/50"
                : "border-transparent text-gray-600 hover:text-gray-800 hover:bg-gray-50"
      ...
      {profileTab === "planos" && (
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 md:p-8 max-w-3xl">
          <h2 className="text-xl font-semibold text-gray-900 mb-1">Convênios Aceitos</h2>
          <p className="text-sm text-gray-500 mb-6">
            Liste os planos de saúde que você atende. O bot do WhatsApp usará essa lista para responder
            pacientes que perguntarem sobre cobertura, e o agendamento exibirá as opções ao marcar consulta.
          </p>
