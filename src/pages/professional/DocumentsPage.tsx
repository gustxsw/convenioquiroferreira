...
const DocumentsPage: React.FC = () => {
  ...
  // Private patient search state (for scheduling-style search in documents form)
  ...
  // Load private patients on demand when searching in the form (similar to scheduling)
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        const apiUrl = getApiUrl();
        const response = await fetchWithAuth(
          `${apiUrl}/api/private-patients?q=${encodeURIComponent(
            privatePatientSearch
          )}&limit=50`,
          {
            headers: {
              "Content-Type": "application/json",
        ...
        if (response.ok) {
          ...
          // We keep the full patients list (for filters and submit) and rely on
          // the backend to return consistent IDs when searching.
          // No extra state needed here; the dropdown uses the same `patients`
          // array already loaded, filtered by the server when needed.
          setPatients(Array.isArray(patientsData) ? patientsData : []);
        }
      } catch (error) {
        // Silent failure for search
      } finally {
        setIsLoadingPatients(false);
      }
    }, 300);
          ...
          <div className="bg-white rounded-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleSubmit} className="p-6">
              <div className="space-y-6">
                ...
                {/* Private Patient Selection - search style (same experience as scheduling/prontuário) */}
                {formData.patient_type === "private" && (
                          ...
                          {isLoadingPatients ? (
                            ...
                            patients.map((patient) => (
                              <button
                                key={patient.id}
                                type="button"
                                onClick={() => handlePrivatePatientSelect(patient)}
