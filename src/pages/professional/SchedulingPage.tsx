...
const SchedulingPage: React.FC = () => {
  ...
  // Generate WhatsApp link with confirmation message
  const generateWhatsAppLink = (appointment: any) => {
    if (!appointment.client_phone) return '#';
    
    // Clean phone number and add Brazil code
    const cleanPhone = appointment.client_phone.replace(/\D/g, '');
    const fullPhone = `5564${cleanPhone}`; // 55 (Brazil) + 64 (Goiás) + phone
    ...
    // Create message
    const message = `Olá ${appointment.client_name}, gostaria de confirmar o seu agendamento com ${user?.name} no dia ${formattedDate} às ${formattedTime}`;
                      ...
                      {appointment ? (
                        <div className="flex items-center justify-between w-full">
                          ...
                          <div className="flex items-center space-x-2">
                            ...
                            {/* WhatsApp Button - only for scheduled appointments */}
