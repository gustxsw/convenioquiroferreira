...
const SchedulingPage: React.FC = () => {
  ...
  const [mobileBlockTime, setMobileBlockTime] = useState("");
  ...
  const handleMobileBlockToggle = async () => {
    if (!mobileBlockTime) {
      return;
    }

    const consultation = consultationsByTime[mobileBlockTime];
    if (consultation) {
      setError("Este horário já possui consulta agendada");
      setTimeout(() => setError(""), 3000);
      return;
    }

    await toggleBlockSlot(mobileBlockTime);
  };
                ...
                <div className="p-4 space-y-3">
                  <div className="border rounded-lg p-4 bg-white shadow-sm">
                    ...
                    <div className="flex flex-col gap-2">
                      <select
                        value={mobileBlockTime}
                        onChange={(e) => setMobileBlockTime(e.target.value)}
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {blockedSlots.map((slot) => (
                          <button
                            key={slot.id}
                            type="button"
                            onClick={() => toggleBlockSlot(slot.time_slot)}
                            className="px-2 py-1 text-xs bg-red-50 text-red-700 rounded-full border border-red-200"
                          >
                            {slot.time_slot} ✕
                          </button>
                        ))}
