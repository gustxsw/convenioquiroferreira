...
app.get("/api/consultations/agenda", authenticate, authorize(["professional"]), async (req, res) => {
  try {
    ...
    // Get phone numbers for WhatsApp functionality
    const consultationsWithPhone = await Promise.all(
      result.rows.map(async (consultation) => {
        let phone = null;
        
        if (consultation.user_id) {
          // Get client phone
          const clientPhone = await pool.query(
            "SELECT phone FROM users WHERE id = $1",
            [consultation.user_id]
          );
          phone = clientPhone.rows[0]?.phone;
        } else if (consultation.dependent_id) {
          // Get client phone (dependent doesn't have own phone)
          const dependentClient = await pool.query(
            `SELECT u.phone FROM users u 
             JOIN dependents d ON u.id = d.user_id 
             WHERE d.id = $1`,
            [consultation.dependent_id]
          );
          phone = dependentClient.rows[0]?.phone;
        } else if (consultation.private_patient_id) {
          // Get private patient phone
          const patientPhone = await pool.query(
            "SELECT phone FROM private_patients WHERE id = $1",
            [consultation.private_patient_id]
          );
          phone = patientPhone.rows[0]?.phone;
        }
